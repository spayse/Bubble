...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Hydrocarbon"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::C8H, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert Hydrocarbon:// to Hydrocarbon:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Hydrocarbon://"))
    {
        uri.replace(0, 12, "Hydrocarbon:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Hydrocarbon.lnk";
}
...
{
    return GetAutostartDir() / "Hydrocarbon.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Hydrocarbon-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Hydrocarbon-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Hydrocarbon-Qt"));
